...
contract PIBP2PPackableRequest is PNFTokenReceiver, ERC223ReceivingContract {
    ...
    function offer(
        ...
        returns (bytes32)
    {
        require(controller.isPNFToken(_tokens[1]), "042");
        require(controller.isToken(_tokens[0]), "043");
    ...
    function deal(bytes32 _offerId, uint _buyAmount) external payable returns (bytes32) {
        ...
        require(!_isPNFTExpired(offers[_offerId].buyToken, offers[_offerId].tokenId), "105");
        ...
        _chargePNFToken(offers[_offerId].buyToken, msg.sender, offers[_offerId].tokenId, _buyAmount);
    ...
    function cancelOffer(bytes32 _offerId) external {
        ...
        bool _isBuyFiat = offers[_offerId].isBuyFiat;

        _updateOffer(_offerId, 0, 0, 0);

        if (!_isBuyFiat) {
            _transfer(_tokenAddress, _sellAmount.add(_commission), msg.sender);
        }
    ...
    function _finishDeal(bytes32 _dealId) private {
        ...
        _transferPNFT(_buyToken, _tokenId, _buyAmount, _seller);
    }
    ...
    function _cancelDeal(bytes32 _dealId) private {
        ...
        _transferPNFT(_buyToken, _tokenId, _buyAmount, _buyer);
